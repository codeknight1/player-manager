          ...
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            <div className="flex flex-wrap justify-between gap-3 p-4">
              <div className="flex min-w-72 flex-col gap-3">
                <p className="text-white tracking-light text-[32px] font-bold leading-tight">Trials & Tournaments</p>
                <p className="text-[#92adc9] text-sm">Register and send invites to players</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
              {trials.map(t => (
                <div key={t.id} className="rounded-lg border border-[#324d67] p-5 bg-[#192633]">
                  ...
                  <div className="flex gap-3">
                    <Button onClick={() => register(t.id)} disabled={t.status === "registered"}>Register</Button>
                    <Button variant="secondary" onClick={() => sendInvite(t.id)}>Send Invite</Button>
                  </div>
                </div>
              ))}
            </div>
