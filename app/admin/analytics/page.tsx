            ...
            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="rounded-lg border border-[#324d67] bg-[#192633] p-6">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[#92adc9] text-sm">Total Users</span>
                    <span className="text-white text-lg font-bold">
                      {analytics?.usersByRole?.reduce((sum: number, item: any) => sum + item._count, 0) || 0}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-[#324d67] bg-[#192633] p-6">
                <h3 className="text-white text-lg font-bold mb-4">Applications by Status</h3>
                <div className="flex flex-col gap-3">
                  {analytics?.applicationsByStatus?.map((item: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-center">
                      <span className="text-[#92adc9] text-sm">{item.status}</span>
                      <span className="text-white text-lg font-bold">{item._count}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="rounded-lg border border-[#324d67] bg-[#192633] p-6">
                <h3 className="text-white text-lg font-bold mb-4">Verifications by Status</h3>
                <div className="flex flex-col gap-3">
                  {analytics?.verificationsByStatus?.map((item: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-center">
                      <span className="text-[#92adc9] text-sm">{item.status}</span>
                      <span className="text-white text-lg font-bold">{item._count}</span>
                    </div>
              ...
              <div className="rounded-lg border border-[#324d67] bg-[#192633] p-6">
                <h3 className="text-white text-lg font-bold mb-4">Recent Tournaments</h3>
                <div className="flex flex-col gap-2">
                  {analytics?.trialsByMonth?.slice(0, 5).map((trial: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-center">
                      <span className="text-[#92adc9] text-xs truncate">{trial.title}</span>
                ...
                <table className="w-full min-w-[500px]">
                  <thead>
                    <tr className="bg-[#192633]">
                      <th className="px-2 sm:px-4 py-3 text-left text-white text-xs sm:text-sm font-medium">Name</th>
                      <th className="px-2 sm:px-4 py-3 text-left text-white text-xs sm:text-sm font-medium hidden sm:table-cell">Email</th>
                      <th className="px-2 sm:px-4 py-3 text-left text-white text-xs sm:text-sm font-medium">Role</th>
                      <th className="px-2 sm:px-4 py-3 text-left text-white text-xs sm:text-sm font-medium hidden md:table-cell">Joined</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics?.recentSignups?.length > 0 ? (
                      analytics.recentSignups.map((user: any, idx: number) => (
                        ...
                        >
                          <td className="px-2 sm:px-4 py-3 text-white text-xs sm:text-sm">{user.name || "No name"}</td>
                          <td className="px-2 sm:px-4 py-3 text-[#92adc9] text-xs sm:text-sm hidden sm:table-cell">{user.email}</td>
