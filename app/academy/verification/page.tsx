...
import { HouseIcon, TrophyIcon, UserIcon, HandshakeIcon, UsersThreeIcon, ShieldCheckIcon, PaperclipIcon } from "@/components/icons";
...
const sidebarItems = [
  ...
  { label: "Verification", href: "/academy/verification", icon: <ShieldCheckIcon size={24} /> },
];
...
export default function AcademyVerificationPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("academy_docs");
    if (saved) setDocs(JSON.parse(saved));
  }, []);
  useEffect(() => { localStorage.setItem("academy_docs", JSON.stringify(docs)); }, [docs]);
    ...
    <div className="relative flex h-auto min-h-screen w-full flex-col bg-[#111a22] overflow-x-hidden" style={{ fontFamily: 'Manrope, "Noto Sans", sans-serif' }}>
      <div className="layout-container flex h-full grow flex-col">
        <div className="gap-1 px-6 flex flex-1 justify-center py-5">
          <Sidebar title="ProConnect" subtitle="Verification" items={sidebarItems} />
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            <div className="flex flex-wrap justify-between gap-3 p-4">
              <div className="flex min-w-72 flex-col gap-3">
                <p className="text-white tracking-light text-[32px] font-bold leading-tight">Verification</p>
                <p className="text-[#92adc9] text-sm">Upload documents to verify your academy or organization</p>
              </div>
            </div>
              ...
              <div className="rounded-lg border border-[#324d67] bg-[#192633] p-4 mb-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  >
                    ...
                    <option>Academy Accreditation</option>
