...
export async function PATCH(
  ...
  try {
    ...
    if (role !== "ADMIN") {
      return NextResponse.json(
        { error: "Forbidden: Only super admins can update trials" },
        { status: 403 }
      );
    }
    ...
    console.error("Error updating trial:", e);
    return NextResponse.json(
      {
...
export async function DELETE(
  ...
  try {
    ...
    if (role !== "ADMIN") {
      return NextResponse.json(
        { error: "Forbidden: Only super admins can delete trials" },
        { status: 403 }
      );
    }
    ...
    console.error("Error deleting trial:", e);
    return NextResponse.json(
      {
